...
pub fn run() {
    // Keep stdout untouched until the whole hook path has completed. A panic
    // in config resolution or auditing must be a silent successful no-op.
    let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if let Some(body) = run_inner() {
            emit(&body);
        }
    }));
}

fn run_inner() -> Option<String> {
    ...
    let (mode, root, repo_mode) = match git::rev_parse(&cwd, "--show-toplevel") {
        Ok(root) if !root.is_empty() => {
            // The reference always invokes audit with ROOT. Its fd discovery
            // therefore ignores a linked-worktree root, where .git is a file.
            (AuditMode::Sweep(PathBuf::from(&root)), root, true)
        }
        _ => {
    ...
    mode_for_root(repo_mode, &root, report)
}
...
fn resolve_timeout_secs() -> u64 {
    ...
    config::resolve(
        &None,
        &["JEEVES_HOOK_TIMEOUT", "WORKTREE_AUDIT_HOOK_TIMEOUT"],
        &config,
        "15",
    )
    .parse()
...
fn audit_with_timeout(mode: AuditMode, opts: AuditOpts, timeout_secs: u64) -> Result<String, ()> {
    if timeout_secs == 0 {
        return Ok(audit_mode(mode, &opts));
    }

    let (sender, receiver) = mpsc::channel();
    thread::spawn(move || {
        let report = audit_mode(mode, &opts);
...
fn audit_mode(mode: AuditMode, opts: &AuditOpts) -> String {
    match mode {
